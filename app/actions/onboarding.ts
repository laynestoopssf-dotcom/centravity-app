...
// =============================================================================
// Default product-line catalog for brand-new agencies (State Farm sub-lines).
// -----------------------------------------------------------------------------
// Written into agencies.custom_product_lines on first creation only (see the
// isNewAgency insert branch below), in the EXACT { name, parent } shape
// Settings -> Custom Product Lines writes when an owner adds one by hand
// (components/SettingsTab.tsx's "Add" button) - same column, same JSON
// shape, so every consumer of custom_product_lines (Scoreboard logging
// dropdowns, resolveParentLine roll-ups, commission/YTD math, the Coaching
// Suite, etc.) picks these up identically to a manually-added line, no
...
const DEFAULT_STATE_FARM_PRODUCT_LINES: { name: string; parent: string }[] = [
  // Auto Roll-Up
  ...
  try {
    ...
    if (agencyId) {
      ...
      const { data: agency, error: agencyError } = await supabaseAdmin
        .from("agencies")
        .insert([{ name: payload.agencyName.trim(), custom_product_lines: DEFAULT_STATE_FARM_PRODUCT_LINES }])
        .select()
        .single();
