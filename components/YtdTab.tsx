    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-300 pb-12">
      ...
      {ytdOverviewData.locations.map((locData: any, idx: number) => (
        <div key={`lob-${idx}`} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 mt-6">
          ...
          {/* Run Rate below is GROSS production pacing (locData.runRateGross*Apps - raw apps
              written, lapse/cancellation completely ignored) against the total production goal.
              This intentionally differs from the Lap/Can-adjusted "Net YTD Apps" shown just above
              it, and from the 2027 AEC Pacing Scorecard in RevenueTab.tsx (which still uses the net
              "gain pacing" runRate*Apps fields, unchanged, since AEC bonus eligibility is meant to
              track net policy growth, not raw production). */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            <div className="p-5 bg-gray-50 rounded-xl border border-gray-200 flex flex-col justify-between">
              ...
              <div><div className="flex justify-between text-xs font-bold text-gray-500 mb-1"><span>Run Rate: {locData.runRateGrossAutoApps}</span><span>Goal: {locData.targets.autoApps}</span></div><div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden"><div className="bg-blue-500 h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(100, (locData.runRateGrossAutoApps / (locData.targets.autoApps || 1)) * 100)}%` }} /></div></div>
            </div>
            <div className="p-5 bg-gray-50 rounded-xl border border-gray-200 flex flex-col justify-between">
              ...
              <div><div className="flex justify-between text-xs font-bold text-gray-500 mb-1"><span>Run Rate: {locData.runRateGrossFireApps}</span><span>Goal: {locData.targets.fireApps}</span></div><div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden"><div className="bg-red-500 h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(100, (locData.runRateGrossFireApps / (locData.targets.fireApps || 1)) * 100)}%` }} /></div></div>
            </div>
            <div className="p-5 bg-gray-50 rounded-xl border border-gray-200 flex flex-col justify-between">
              ...
              <div><div className="flex justify-between text-xs font-bold text-gray-500 mb-1"><span>Run Rate: {locData.runRateGrossCommercialApps}</span><span>Goal: {locData.targets.commercialApps}</span></div><div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden"><div className="bg-amber-500 h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(100, (locData.runRateGrossCommercialApps / (locData.targets.commercialApps || 1)) * 100)}%` }} /></div></div>
            </div>
            <div className="p-5 bg-gray-50 rounded-xl border border-gray-200 flex flex-col justify-between">
              ...
              <div><div className="flex justify-between text-xs font-bold text-gray-500 mb-1"><span>Run Rate: {locData.runRateGrossHealthApps}</span><span>Goal: {locData.targets.healthApps}</span></div><div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden"><div className="bg-emerald-500 h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(100, (locData.runRateGrossHealthApps / (locData.targets.healthApps || 1)) * 100)}%` }} /></div></div>
            </div>
          </div>
        </div>
      ))}
