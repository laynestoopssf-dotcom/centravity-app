...
const WhatIfModeToggle = ({ mode, setMode }: { mode: WhatIfMode; setMode: (m: WhatIfMode) => void }) => (
  ...
  <div className="relative self-start flex items-center bg-gray-100 dark:bg-slate-700 rounded-full p-1 text-xs font-bold" onClick={e => e.stopPropagation()}>
    <div
      className="absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-full bg-white dark:bg-slate-900 shadow-sm transition-transform duration-300 ease-out"
      style={{ transform: mode === 'ytd' ? 'translateX(0%)' : 'translateX(calc(100% + 8px))' }}
    />
    <button
      type="button"
      onClick={() => setMode('ytd')}
      className={`relative z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors duration-200 ${mode === 'ytd' ? 'text-blue-700' : 'text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-300'}`}
    >
    ...
    <button
      type="button"
      onClick={() => setMode('mtd')}
      className={`relative z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors duration-200 ${mode === 'mtd' ? 'text-blue-700' : 'text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-300'}`}
    >
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-2xl border shadow-sm transition-colors duration-300 ${theme.bg} ${theme.border}`}>
      ...
      <div className={`relative flex items-center w-64 h-11 rounded-full p-1 transition-colors duration-300 ${theme.accent}`}>
        <div
          className="absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-full bg-white dark:bg-slate-900 shadow-md transition-transform duration-300 ease-out"
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-300 pb-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100">Agency Overview</h2>
          <p className="text-gray-500 dark:text-slate-400 mt-1">Month-to-Date performance across the entire team.</p>
        </div>
        
        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-2 pl-4 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
          <span className="text-xs font-bold text-gray-400 dark:text-slate-400 uppercase flex items-center gap-2"><CalendarDays size={16}/> Target Month:</span>
          <input 
            type="month" 
            value={overviewMonth} 
            onChange={(e) => {
            ...
            className="p-1.5 bg-gray-50 dark:bg-slate-800 border border-gray-300 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-blue-600"
          />
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm relative">
          <h4 className="text-sm font-semibold text-gray-500 dark:text-slate-400 mb-2">AGENCY PREMIUM (MTD · {dateFilterMode === 'issued' ? 'Issued' : 'Written'})</h4>
          <div className="flex items-end gap-3"><div className="text-4xl font-black text-gray-900 dark:text-slate-100">${agencyOverviewData.totals.monthPremium.toLocaleString()}</div></div>
          {premPacing.isValid && (
            <div className="mt-3 flex items-center gap-1 text-sm font-medium">
              ...
              <span className="text-gray-400 dark:text-slate-400 ml-1">vs prior mo (pace)</span>
            </div>
          )}
        </div>

        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm relative">
          <h4 className="text-sm font-semibold text-gray-500 dark:text-slate-400 mb-2">BOUND APPS (MTD · {dateFilterMode === 'issued' ? 'Issued' : 'Written'})</h4>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm relative">
          <h4 className="text-sm font-semibold text-gray-500 dark:text-slate-400 mb-2">QUOTES (MTD)</h4>
          <div className="text-4xl font-black text-purple-600">{agencyOverviewData.totals.monthQuotes || 0}</div>
          <p className="text-[11px] text-gray-400 dark:text-slate-400 mt-1">Sourced from logged activities &middot; not affected by the Written/Issued toggle</p>
          {quotePacing.isValid && (
            <div className="mt-3 flex items-center gap-1 text-sm font-medium">
              {quotePacing.isUp ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm relative">
          <h4 className="text-sm font-semibold text-gray-500 dark:text-slate-400 mb-2">TOUCHES (MTD)</h4>
      ...
      <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100 mt-8 mb-4 flex items-center gap-2"><Sparkles size={20} className="text-blue-500"/> Month-End Projections</h3>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gray-900 text-white p-6 rounded-2xl border border-gray-800 shadow-sm relative overflow-hidden">
          ...
          <h4 className="text-xs font-bold text-gray-400 dark:text-slate-400 mb-2 uppercase tracking-wider">Projected Premium</h4>
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mt-8">
        <div className="flex items-center gap-2">
          <ShieldCheck size={20} className="text-gray-400 dark:text-slate-400" />
          <h4 className="text-sm font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Global Bound Lines</h4>
        </div>
        <div className="flex flex-wrap gap-3">
          {Object.entries(globalLines).map(([line, count]) => (
            <div key={line} className={`px-4 py-2 rounded-xl border flex items-center gap-3 ${count > 0 ? 'bg-blue-50 border-blue-100' : 'bg-gray-50 dark:bg-slate-800 border-gray-100 dark:border-slate-800 opacity-70'}`}>
              <span className={`font-semibold text-sm ${count > 0 ? 'text-blue-900' : 'text-gray-500 dark:text-slate-400'}`}>{line}</span>
              <span className={`px-2.5 py-0.5 rounded-md text-sm font-black ${count > 0 ? 'bg-white dark:bg-slate-900 text-blue-700 shadow-sm' : 'bg-gray-200 text-gray-400 dark:text-slate-400'}`}>{count}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-purple-50/60 border border-purple-100 rounded-2xl p-5">
          ...
          <div className="flex flex-col gap-3">
            {agencyOverviewData.ownerRows.map((member: any) => (
              <div key={member.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 border border-purple-100 rounded-xl px-4 py-3 shadow-sm">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-400">
                      ...
                      <span className="font-bold text-gray-700 dark:text-slate-200">{member.closeRate}%</span> close rate
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex flex-wrap gap-1.5">
                    <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded px-1.5 py-0.5">Auto <span className="text-gray-800">{member.linesBreakdown?.Auto || 0}</span></span>
                    <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded px-1.5 py-0.5">Fire <span className="text-gray-800">{member.linesBreakdown?.Fire || 0}</span></span>
                    <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded px-1.5 py-0.5">Life <span className="text-gray-800">{member.linesBreakdown?.Life || 0}</span></span>
                    <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded px-1.5 py-0.5">Health <span className="text-gray-800">{member.linesBreakdown?.Health || 0}</span></span>
                    <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded px-1.5 py-0.5">Commercial <span className="text-gray-800">{member.linesBreakdown?.Commercial || 0}</span></span>
                  </div>
                  <p className="text-lg font-black text-gray-900 dark:text-slate-100 shrink-0">${member.monthPremium.toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800 flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">Producer Leaderboard</h3>
              ...
              {agencyOverviewData.leaderboard.map((member: any, index: number) => {
                ...
                return (
                  <React.Fragment key={member.id}>
                    <tr onClick={() => setExpandedProducerId(isExpanded ? null : member.id)} className={`transition-colors cursor-pointer ${isExpanded ? 'bg-blue-50/50' : 'hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2.5">
                          <ProfileAvatar src={member.avatar_url} name={displayName} stealth={isStealth} size="sm" />
                          <div>
                            <div className="font-bold text-gray-900 dark:text-slate-100">{displayName}</div>
                            {!isStealth && <div className="text-xs text-gray-400 dark:text-slate-400 capitalize">{member.role}</div>}
                          </div>
                        </div>
                      </td>
                      
                      <td className="px-6 py-5"><div className="flex items-center gap-3"><span className="font-semibold text-gray-900 dark:text-slate-100 w-8">{member.monthTouches}</span><div className="w-16 h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden"><div className={`h-full rounded-full ${getPacingColor(tPacing)}`} style={{ width: `${Math.min(100, tPacing)}%` }} /></div></div></td>
                      <td className="px-6 py-5"><div className="flex items-center gap-3"><span className="font-semibold text-gray-900 dark:text-slate-100 w-8">{member.monthQuotes}</span><div className="w-16 h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden"><div className={`h-full rounded-full ${getPacingColor(qPacing)}`} style={{ width: `${Math.min(100, qPacing)}%` }} /></div></div></td>
                      <td className="px-6 py-5"><div className="flex items-center gap-3"><span className="font-semibold text-gray-900 dark:text-slate-100 w-8">{member.monthBound}</span><div className="w-16 h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden"><div className={`h-full rounded-full ${getPacingColor(bPacing)}`} style={{ width: `${Math.min(100, bPacing)}%` }} /></div></div></td>
                      
                      <td className="px-6 py-5 text-right font-black text-gray-900 dark:text-slate-100">${member.monthPremium.toLocaleString()}</td>
                      <td className="px-6 py-5 text-right"><span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-200">{member.closeRate}%</span></td>
                      <td className="px-4 py-5 text-gray-400 dark:text-slate-400 text-right">{isExpanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</td>
                    </tr>
                    
                    {isExpanded && (
                      <tr className="bg-gray-50 dark:bg-slate-800 border-t-0">
                        <td colSpan={7} className="px-6 py-4 pb-6 border-b border-gray-100 dark:border-slate-800">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                              <div className="flex items-center gap-2 mb-3"><span className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Product Line Breakdown (Bound)</span></div>
                              <div className="flex flex-wrap gap-3">
                                {Object.entries(member.linesBreakdown).map(([line, count]: any) => (
                                  <div key={line} className={`px-4 py-2 rounded-xl border flex items-center gap-3 ${count > 0 ? 'bg-white dark:bg-slate-900 border-blue-100 shadow-sm' : 'bg-gray-100 dark:bg-slate-700 border-gray-200 dark:border-slate-800 text-gray-400 dark:text-slate-400 opacity-60'}`}>
                                    <span className={`font-semibold text-sm ${count > 0 ? 'text-gray-900 dark:text-slate-100' : 'text-gray-500 dark:text-slate-400'}`}>{line}</span>
                                    <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-500 dark:text-slate-400'}`}>{count}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                            ...
                            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
                              <div className="flex flex-col gap-3 mb-3">
                                <div className="flex justify-between items-center">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs text-gray-500 dark:text-slate-400 font-semibold">Goal Commission:</span>
                                    <div onClick={e => e.stopPropagation()}>
                                      <FormattedNumberInput value={whatIfCommission} onChange={(v) => setWhatIfCommission(v === '' ? 0 : v)} className="w-24 p-1.5 bg-gray-50 dark:bg-slate-800 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-600 text-sm font-bold" />
                                    </div>
                                  </div>
                                </div>
                              ...
                              {activeWhatIf && (
                                <div className="mt-3 flex items-center justify-between text-[11px] text-gray-400 dark:text-slate-400 font-medium">
                                  <span>Close Rate: <strong className="text-gray-600 dark:text-slate-300">{activeWhatIf.closeRate}%</strong></span>
                                  <span>Quote Rate: <strong className="text-gray-600 dark:text-slate-300">{activeWhatIf.quoteRate}%</strong></span>
                                  <span>$/App: <strong className="text-gray-600 dark:text-slate-300">${Math.round(activeWhatIf.commissionPerApp).toLocaleString()}</strong></span>
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="mt-6 bg-gradient-to-br from-indigo-50 to-purple-50 p-5 rounded-xl border border-indigo-100 shadow-sm">
                            ...
                            {aiInsights[member.id] ? (
                              <div className="bg-white/80 dark:bg-slate-700/80 p-4 rounded-lg border border-indigo-100 text-sm text-indigo-900 font-medium leading-relaxed shadow-sm">
                                {aiInsights[member.id]}
                              </div>
                            ) : (
